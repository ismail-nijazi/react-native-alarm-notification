...
ReactNativeAN.scheduleAlarm = async (details) => {
	if (
		...
		throw new Error(
			"failed to schedule alarm because fire date is missing"
		);
	}

	const past = parseDateString(
		details.fire_date
	);
	...
	const repeatInterval =
		details.repeat_interval || "hourly";
	const intervalValue =
		details.interval_value || 1;
	...
	if (
		...
		throw new Error(
			"interval value should be between 1 and 59 minutes"
		);
	}
	...
	if (
		...
		throw new Error(
			"interval value should be between 1 and 23 hours"
		);
	}
	...
	const data = {
		...
		vibrate:
			typeof details.vibrate === "undefined"
				? true
				: details.vibrate,
		play_sound:
			typeof details.play_sound ===
			"undefined"
				? true
				: details.play_sound,
		schedule_type:
			details.schedule_type || "once",
		repeat_interval:
			details.repeat_interval || "hourly",
		interval_value:
			details.interval_value || 1,
...
ReactNativeAN.deleteAlarm = (id) => {
	if (!id) {
		throw new Error(
			"id is required to delete alarm"
		);
	}
...
ReactNativeAN.removeFiredNotification = (id) => {
	if (!id) {
		throw new Error(
			"id is required to remove notification"
		);
	}

	RNAlarmNotification.removeFiredNotification(
		id
	);
};
...
ReactNativeAN.requestPermissions = async (
	permissions
) => {
	...
	return await RNAlarmNotification.requestPermissions(
		requestedPermissions
	);
};
...
ReactNativeAN.checkPermissions = (callback) => {
	RNAlarmNotification.checkPermissions(
		callback
	);
};
...
ReactNativeAN.parseDate = (rawDate) => {
	...
	if (
		rawDate.getMonth().toString().length === 1
	) {
